...
def generate_images(prompts_path, save_path, device='cuda:0', guidance_scale=3.0, image_size=512, ddim_steps=25, num_samples=1, from_case=0, concept='jolie'):
    '''
    ...
    The program requires the prompts to be in a csv format with headers 
        ...
        3. 'evaluation_seed' (the inital seed to generate gaussion noise for diffusion input)
        4. 'evaluation_guidance' (the scale for classifier-free guidance)
    ...
    '''
    diffusers_dir = "stable-diffusion/diffusers_ckpt/ORI" # the original SD path (diffusers version)
        
    # 1. Load the autoencoder model which will be used to decode the latents into image space.
    vae = AutoencoderKL.from_pretrained(diffusers_dir, subfolder='vae')
    # 2. Load the tokenizer and text encoder to tokenize and encode the text.
    tokenizer = CLIPTokenizer.from_pretrained(diffusers_dir, subfolder='tokenizer')
    text_encoder = CLIPTextModel.from_pretrained(diffusers_dir, subfolder='text_encoder')
    # 3. The UNet model for generating the latents.
    unet = UNet2DConditionModel.from_pretrained(diffusers_dir, subfolder="unet")
    ...
    for i, row in df.iterrows():
        ...
        print(f"case: {case_number}, seed: {seed}, prompt: {prompt}")
...
if __name__=='__main__':
    ...
    parser.add_argument('--device', help='cuda device to run on', type=str, required=False, default='cuda:2')
    parser.add_argument('--guidance_scale', help='guidance to run eval', type=float, required=False, default=3.0)
    ...
    generate_images( prompts_path, save_path, device=device,
                    guidance_scale = guidance_scale, image_size=image_size, ddim_steps=ddim_steps, num_samples=num_samples,from_case=from_case, concept=concept)
